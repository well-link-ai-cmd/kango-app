...
2. 訪問メモ内に出てくる本人・家族の発言は corrected_s_input に入れず、O に「本人より〜との訴えあり」「家族より〜との報告あり」と客観記載する
3. 過去記録の S 欄・【AIからの確認質問への回答】【前回からの継続確認事項への回答】の本文・「特になし」等のプレースホルダを corrected_s_input に混ぜてはならない
4. 【S情報】（本人・家族の発言）は A/P の臨床判断材料として必ず考慮する（例「痛みが増した」→ A で疼痛増悪評価、P でレスキュー検討）
...
3. O・A・P：coverage_check に従って記述。extracted_facts の事実を全て反映する（S欄は出力しない）
