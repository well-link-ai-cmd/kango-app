...
## 引き継ぎ（最終更新: 2026-05-15 — 看護ケアUI改修 + 誤字補正 master投入完了）

### 次回再開時の最優先タスク
1. **看護ケアリストUIの実運用FB（PR #12 改修後）**：本番で追加・削除候補の動作確認
...
2. **実運用フィードバック収集（PR #11 改修後の品質チェック）**：何件か作ってみて以下を確認
...
3. **音声誤変換FB（PR #13 反映後）**：併願→閉眼／肉毛→肉芽／色・慰労・要ろう→胃瘻／外相→咳嗽 が補正されているか実運用で確認。新たな誤変換パターンがあれば次回まとめて追加
4. **改善判断**：実運用FBで品質に納得いかなければ Few-shot 化を検討
