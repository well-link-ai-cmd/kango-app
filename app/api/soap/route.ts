...
${rawInput}`;
  ...
  try {
    ...
    // 誤変換補正版（corrected_s_input）が元の8割以上の長さを保っていれば採用し、
    // 極端に短い＝簡略化された場合は生の S情報をそのまま使ってデータ欠落を防ぐ。
    let S = "";
    if (hasSInput) {
      const raw = (sInput as string).trim();
      const corrected = (ti.corrected_s_input ?? "").trim();
      // 補正版を採用する条件：(1) 簡略化されていない（元の8割以上の長さ）かつ
      // (2) 話者ラベル（S:/妻S:/娘S: 等）が入力と完全一致している。
      // どちらか崩れていれば、補正より「入力どおりのS（ラベル・内容の完全保持）」を優先して生のS情報を使う。
      ...
      S = corrected.length >= raw.length * 0.8 && labelsPreserved ? corrected : raw;
    }
