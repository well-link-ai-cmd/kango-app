...
A：主病名や問題点ごとに段落を分け、「〜に関しては、」のように問題名を冒頭に挙げてからアセスメントを記述する。各問題の現状評価・前回からの変化・今後の方向性を含める。番号リストは使わない。
P：今後のケア方針を自然な文章で3〜5文にまとめる。「〜していく」「〜を継続する」の語尾で統一。番号リスト・見出しは使わない。

【重要】
・メモから要点を抽出して整理する。勝手に事実を追加しない
・前回記録のプランに記載された継続事項を今回のA・Pに反映する
・★最重要★ 過去の訪問記録が提供されている場合は、そのS/O/A/Pの文体・構成・詳細度・文章量を忠実に模倣する。特にAセクションの書き方（問題点の挙げ方、段落構成、表現パターン）は過去記録のスタイルに厳密に合わせる
