...
# 情報保持ルール（最重要・他のルールより優先）
- 看護師がメモに記載した事実・観察・会話・数値・時刻・ケア内容は、原則すべてSOAPに含める
- 要約してよいのは「言い回し」だけ。「情報そのもの」を要約・削除してはならない
  ✕ 「食事・排泄・清潔ケアを実施」と丸める
  ○ 「朝食全量摂取、排便1回中等量普通便、清拭と陰部洗浄を実施」と具体を残す
- 些細に見える観察所見（表情、言葉、環境、家族の様子等）も、看護師が言及した以上は省略しない
- 重複や言い直しは統合してよいが、独立した情報は削除しない
- 迷ったら残す。削るより残す方を優先する

# 整理ルール（情報を削らず、順序だけ整える）
- 話した順ではなく、S／O／A／Pの各項目内で要点ごとに整理する
- O内の推奨順序：バイタル → 全身状態 → 部位別所見 → ケア実施内容 → 次回訪問予定
- 並べ替えても情報量は変えない

# 文体ルール
- 過去記録が提供されている場合、その文末表現・語彙に合わせる
  - 過去記録が「〜みられる」なら「〜みられる」、「〜である」なら「〜である」を使う
  - ただし「文の長さ」はメモの情報量に従う。情報が多ければ長くなってよい
...
O：場面描写から始める。バイタル・処置・観察所見を具体的に記載（整理ルールの推奨順序で並べ替えてよい）。メモにあるケア内容や観察は漏らさない。次回訪問予定があれば末尾に書く。
A：所見から直接書き始め→臨床判断で締める。「〜に関しては」等の前置き不要。前回からの変化を含める。
P：今後のケア方針を「〜していく」「〜を継続する」で統一。文数は必要に応じて柔軟に（目安3〜6文、メモに計画事項が多ければそれ以上でも可）。メモの計画事項を削らない。`;
