...
# 絶対ルール
- alerts は過去記録（看護計画書・前回P・登録ケア内容）由来の項目のみ。今日のメモが曖昧な点を掘り下げる質問は出さない（看護師の負担になるため）
    ...
    input_schema: {
      ...
      properties: {
        ...
        expected_from_context: {
          type: "array",
          items: { type: "string" },
          description: "看護計画書・前回P・次回確認事項・登録ケア内容から、今日確認/実施が期待される項目を列挙。内部確認用。",
        },
        alerts: {
          type: "array",
          items: { type: "string" },
          description: "expected_from_context のうち memo_covers に該当がないもの。最大3件。『前回P継続：〜が記載されていない』『登録ケア内容：〜の実施記載がない』形式。該当なしは空配列。",
        },
      },
