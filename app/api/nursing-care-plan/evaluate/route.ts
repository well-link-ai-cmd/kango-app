        ...
        evaluations: {
          ...
          items: {
            ...
            properties: {
              ...
              evaluation: {
                type: "string",
                description: "評価本文（300〜500字程度の自然な自由文・1ブロック）。バイタル・症状・ADL・実施ケア・活動量・服薬の状況を時系列または項目順に簡潔に記述し、最後に『継続する。』『達成とする。』『見直しを検討する。』等の体言止めの方針を1文添える。見出し（【経過サマリ】等）は付けない。",
              },
            },
...
- 含めるべき要素（該当課題に関連する範囲で）：
  ...
  * 体重・摂取量等の数値（記録にあれば）
- 末尾の方針は**体言止め**で1文：「継続する。」「達成とする。」「計画見直しを検討する。」「経過観察を継続する。」等。「〜していきます」「〜します」のような丁寧語は使わない（看護記録は体言止めが慣習）
