  ...
  const generateTool = {
    ...
    input_schema: {
      ...
      properties: {
        disease_progress: {
          type: "string",
          description: "病状の経過（500〜1000字目安）。バイタル変化・症状変動・治療反応・ADL変化・計画目標に対する評価を時系列で。看護の内容欄で実施ケアを詳述するため、本欄は『状態経過の流れ』に絞る。問題点と評価を具体的に書くが、ケア手技の詳細は書かない。",
        },
        nursing_content: {
          type: "string",
          description: "★看護・リハの内容（800〜1500字、本様式の最重要欄。報告書レベルで詳細に書く）。"
            + "実施した看護を【看護タイトル】形式で 5〜8項目列挙し、各項目に "
            + "(1)実施ケア（手技・頻度・タイミング） "
            + "(2)利用者の反応・観察事項（バイタル・症状・ADL変化など客観的事実） "
            + "(3)看護師としての判断・気づき・継続必要性 "
            + "の3点を含める。他欄（病状経過・家族・特記）と重複する事実は他欄で簡潔に書き、本欄では『看護師として何を観察しどう介入したか』を中心に詳述する。SOAPに記載のあるケアのみ書く。"
            + (isPsych ? "精神科では服薬支援・症状モニタリング・生活リズム支援・SST等の関わりを中心に。" : ""),
        },
        family_care: {
          type: "string",
          description: isPsych
            ? "家族等との関係（300〜600字目安）。家族構成・キーパーソン・関係性の良好/緊張・家族の疾病理解・家族支援の要否。SOAPに記載がある事実のみ。看護師の家族支援ケアは看護の内容欄で書くため、本欄は『家族の状況』に絞る。"
            : "家庭での介護の状況（300〜600字目安）。誰が・どのように介護しているか・介護者の心身状態・介護負担の安定性・キーパーソン。看護師の家族支援ケアは看護の内容欄で書くため、本欄は『介護の状況』に絞る。",
        },
        special_notes: {
          type: "string",
          description: "特記すべき事項（150〜500字目安）。他欄に収まらない重要事項のみ。他欄で既に詳述した内容の繰り返しは禁止。頻回訪問の理由は『SOAPに記載された事実』のみ列挙し、必要性の判断や訪問頻度の提案は行わない。"
            + (isPsych ? "精神科では希死念慮等のリスクがSOAPに記載されている場合は事実として明示。" : ""),
        },
      },
