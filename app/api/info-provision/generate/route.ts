...
const PROMPT_VERSION = "info-provision-v2.0.1"; // v2.0.1: 開示に耐える表現ルール（「述べている」禁止・2026-07-07）。v2.0.0: 事業所カスタムGPTの書き方（です・ます調/簡潔/3段落構成）に準拠
...
- 利用者・家族が読む可能性を前提に、不快・失礼に響く表現を避ける。本人・家族の発言に触れる場合は「〜とのことです」「〜と話されています」を用い、「〜と述べている」「〜と述べています」は使わない
...
# 看護の内容欄について（最重要欄）
- **「看護の内容」欄は本様式の中核**。長さではなく「受入先が看護をイメージできること」を優先する。3段落の構成・文量・各段落の内容は欄のdescription指示に従う
